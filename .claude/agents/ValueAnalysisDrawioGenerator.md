...
- **ステークホルダー**
  ...
  - ステークホルダーの配置
    - 「自社」を一番右に配置する。
    - 「自社」を基準とし、関係性の強いステークホルダーから順に自社の左側へ配置する。関係が強いほど自社に近い位置に置き、関係が弱くなるにつれて外側へ並べる。
  - アイコン：UMLのユースケース図で使用されるアクターのアイコンを用いる

- **価値記述**
  - **【重要】出力位置：該当のステークホルダーのアイコンの真下に並べる（どのステークホルダーの課題なのかを明示する）**
  - **【重要】`output/value-analysis-value-description.tsv` の「価値記述」列の値を要素内に出力する（「価値記述名」列の値は出力不要）**
  - 並び順：価値の重要度が高い順に図の上から出力する
  - アイコン：角丸長方形を使用
  - **図形サイズ**: width="200" height="80" 以上（価値記述の全文が表示できるサイズ）
  - **内容の形式**:
    - 1行目: 「目的アルファベット.価値記述の全文（「〜で嬉しい」形式）」
  - 関連する目的のアルファベットを価値記述の左側に付与（例：「A.<改行コード>月末の経理作業時に、業務フローが簡潔なので、作業工数が3日から1日に短縮されて嬉しい」）
  - **【重要】要素の色：関連する目的と同じ色に設定する**
  - **【重要】価値記述の先頭に関連する目的のアルファベットを記載**
