...
以下の2つの方式により、企画の元となるアイデアを作成します。

* ユーザーがAIからの質問に回答。
* 既存の製品・サービス情報をWebからリサーチ。

### モデルの生成
* 匠Methodのコアモデル4つ（ステークホルダーモデル、価値デザインモデル、価値分析モデル、要求分析ツリー）をDrawIO形式で生成します。
* ビジネスコンテキストフローをDrawIO形式で生成します。
* ゴール記述モデルをTSV形式で生成します。
...
匠Methodで企画を立てる前に、背景、課題、課題解決の案、自社の情報などを記載するファイル。

[アイデア：企画の元となるファイル：samples/furoya-takumi/input/idea.md](samples/furoya-takumi/input/idea.md)
...
モデリングを開始するには、`input/idea.md` に企画の元となるアイデアファイルを用意します。3つの方法があります。
...
URLを指定すると、製品・サービスを分析してアイデアファイルを自動生成します。複数URLも指定可能です。
...
アイデアファイルを準備したら、以下のコマンドで全成果物を一括生成できます。
